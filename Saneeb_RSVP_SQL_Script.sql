...
ORDER BY movie_count DESC
LIMIT 3;Hi my name is asaneeb sheih li
...
-- Step 2: Clean currency and rank movies within each year
movie_gross_summary AS (
    SELECT 
        g.genre,
        m.year,
        m.title AS movie_name,
        -- Handling NULLs and stripping '$' or 'INR ' to make the column numeric
        CAST(REPLACE(REPLACE(IFNULL(worlwide_gross_income, '0'), '$ ', ''), 'INR ', '') AS DECIMAL(20)) AS worldwide_gross_income,
        DENSE_RANK() OVER(
            PARTITION BY m.year 
            ORDER BY CAST(REPLACE(REPLACE(IFNULL(worlwide_gross_income, '0'), '$ ', ''), 'INR ', '') AS DECIMAL(20)) DESC
        ) AS movie_rank
    FROM movie AS m
    INNER JOIN genre AS g ON m.id = g.movie_id
    WHERE g.genre IN (SELECT genre FROM top_3_genres)
)
-- Step 3: Filter for the top 5 movies per year
SELECT * FROM movie_gross_summary
WHERE movie_rank <= 5;
...
WITH actress_summary AS (
    SELECT 
        ...
        -- Weighted average calculation using only superhit movies
...
WITH director_metrics AS (
    SELECT 
        ...
        r.avg_rating,
        r.total_votes,
        -- Added m.id to resolve ambiguity
